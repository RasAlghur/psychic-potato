...
client.on('messageCreate', async (message) => {
    ...
    if (matches) {
        matches.forEach(async (ca) => {
            ...
            if (caTracker[ca]) {
                console.log(`Token address ${ca} already tracked by ${caTracker[ca][0].username}. Ignoring further mentions.`);
                return;
            }
        ...
        if (isIncomplete) {
            ...
            if (marketCap > caTracker[ca].allTimeHigh) {
                ...
                const embed = new EmbedBuilder()
                    .setColor(0xff0000)
                    .setTitle(`${tokenSymbol} just reached a marketCap of ${marketCap}, new all-time high!`)
                    .setThumbnail(BOT_IMAGE_URL)
                    .addFields(
                        { name: 'Caller Profile', value: `${username} (Total Calls: ${totalCallsByUser})`, inline: true },
                        { name: 'Called at', value: initialMarketCap.toString() || 'NA', inline: true },
                        { name: 'ATH MCAP', value: caTracker[ca].allTimeHigh.toString() || 'NA', inline: true }
                    )
                    .setTimestamp();

                alertChannel.send({ embeds: [embed] });
            } else {
                console.log(`${marketCap} MC did not pass ATH of ${caTracker[ca].allTimeHigh}`)
                return;
            }
        }
    }
...
async function checkAndSendAlert(ca) {
    ...
    const { tokenName, tokenSymbol, tokenLogo, marketCap, exchangeValue } = await getTokenMetadata(ca);
    ...
    if (isIncomplete) {
        ...
        const channelMentions = caTracker[ca].reduce((acc, entry) => {
            ...
            acc[entry.channelId].messages.push(`[Message tracked at <t:${Math.floor(entry.timestamp / 1000)}:T>](${entry.messageLink}) by ${entry.username}`);
            return acc;
        }, {});
        ...
        const embed = new EmbedBuilder()
            ...
            .addFields(
                { name: 'Caller Profile', value: `${username} (Total Calls: ${totalCallsByUser})`, inline: true },
                { name: 'Dex', value: `[Dexscreener](https://dexscreener.com/search?q=${ca})`, inline: true },
